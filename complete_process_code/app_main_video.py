...
if __name__ == '__main__':
    ...
    # 使用监测模型一帧一帧对视频进行监测
    # 如果监测模型认为当前帧为正常样本，则继续下一帧
    # 如果监测模型认为当前帧为对抗攻击样本，则停止监测，并且提示对抗攻击风险
    cap = cvs.VideoCapture('/home/intelligent_transportation/AI_test_data/test_adv.avi')
    frame_id = 0
    while True:
        # 获取图像
        frame = cap.read()
        ...
        print(f'frame_id:{frame_id} 监测模型监测得分: {detect_pred.item():.4f}')
        cvs.imshow(frame)  # 绘制图像
        if detect_pred > 0.5:
            id = 'tHyzzn1'
            ...
            result = requests.post(request_url + "id=" + id + "&text=" + text + "&ts=" + ts + "&type=" + type,
                                headers=headers)
            print('喵提醒发送警告: 存在对抗攻击风险！')
            # break
        else:
            print(f'frame_id:{frame_id} 为正常样本！')
            # logits = model(img)
            # print(logits.size())
            
            # pred = imagenet_label2classname(predict_from_logits(logits))
            # print('=' * 60)
            # print('result: ', pred)
